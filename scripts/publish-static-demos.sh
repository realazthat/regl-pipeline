...
if [ "$TRAVIS_REPO_SLUG" == "realazthat/regl-pipeline" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [[ $PUBLISH_BRANCHES =~ "$TRAVIS_BRANCH" ]]; then
  ...
  rm -rf gh-pages
  git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/realazthat/regl-pipeline gh-pages > /dev/null
